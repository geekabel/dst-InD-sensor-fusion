...
class MapSensor(VirtualSensor):
    ...
    def _get_lanelet_subtype(self, x: float, y: float, 
        ...
        if not LANELET2_AVAILABLE or lanelet_map is None or projector is None:
            print("Warning: Using simplified map logic.")
            if y < -5 or y >= 5:
                return "sidewalk"
            elif (-5 <= y < -2) or (2 <= y < 5):
                return "bicycle_lane"
        ...
        try:
            ...
            if nearest_lanelets:
                ...
                if nearest_lanelet.hasAttribute("subtype"):
                    subtype = nearest_lanelet.attributes["subtype"]
                    if subtype in self.subtype_probs:
                        return subtype
                    else:
                        print(f"Warning: Unknown lanelet subtype '{subtype}' found.")
                        return "unknown"
                else:
            ...
            print(f"Error querying Lanelet2 map at ({x}, {y}): {e}")
            return "unknown"
